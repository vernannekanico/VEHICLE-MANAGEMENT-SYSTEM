...
"
    ...
    Private Sub WorkOrdersDataGridView_RowEnter(sender As Object, e As DataGridViewCellEventArgs) Handles WorkOrdersDataGridView.RowEnter
        ...
        If CurrentUserGroup = "Assistant Service Manager" Then
            DisableConcernMenus()
            DisableJobMenus()
        End If
        AssignWorkOrderToolStripMenuItem.Visible = False
        If InStr("For Analysis/For Assignment/", CurrentWorkOrderStatus) > 0 Then
            If InStr("Assistant Service Manager/Lead Service Specialist", CurrentUserGroup) > 0 Then
                AssignWorkOrderToolStripMenuItem.Visible = True
            End If
        End If

        RevertStatusToolStripMenuItem.Visible = False
        If InStr("Assigned/For Assignment/For Analysis", CurrentWorkOrderStatus) > 0 Then
            If InStr("Assistant Service Manager/Lead Service Specialist", CurrentUserGroup) > 0 Then
                RevertStatusToolStripMenuItem.Visible = True
            End If
        End If

        'note following were moved to row enter at concerns
...
" 
    ...
    Private Sub WorkOrderConcernsDataGridView_RowEnter(sender As Object, e As DataGridViewCellEventArgs) Handles WorkOrderConcernsDataGridView.RowEnter
        ...
        AddJobToolStripMenuItem.Visible = False
        If InStr("No Part Needed/No Action Yet/Draft Requisition/Assigned/Parts From Customer", CurrentConcernStatus) Then AddJobToolStripMenuItem.Visible = True
        If IsEmpty(CurrentConcernStatus) Then CurrentConcernStatus = ""
        Select Case CurrentConcernStatus
            Case "Assigned"
                ...
                End If
                'check if personnel is also allowed access
            Case "Draft Requisition"
                EnableAccesToParts(CurrentAssignedServiceSpecialistID)
        ...
        End If
    End Sub
    Public Sub EnableAccesToParts(CurrentAssignedServiceSpecialistID)
...
"
    ...
    Private Sub WorkOrderConcernJobsDataGridView_RowEnter(sender As Object, e As DataGridViewCellEventArgs) Handles WorkOrderConcernJobsDataGridView.RowEnter
        ...
        RemoveJobToolStripMenuItem.Visible = False
        If InStr("No Part Needed/No Action Yet/Draft Requisition", XXCurrentJobStatus) Then RemoveJobToolStripMenuItem.Visible = True
        Select Case XXCurrentJobStatus
            ...
            Case "Assigned"
                ...
                If CurrentPersonelID = CurrentAssignedServiceSpecialistID Then
                    EnableJobMenus()
                End If
                EnableAccesToParts(CurrentAssignedServiceSpecialistID) 'Only CurrentAssignedServiceSpecialistID) can work with parts
            Case "Draft Requisition"
                EditJobToolStripMenuItem.Visible = False
                'NOTE THIS IS DIABLED TO ENABLE INSERT CUSTOMER SUPPLIES
                '            CustomerSuppliedPartsToolStripMenuItem.Visible = False
                If CurrentPersonelID = CurrentAssignedServiceSpecialistID Then RequestPartsFromWarehouseToolStripMenuItem.Visible = True

            Case "All Parts Received"
                RemoveJobToolStripMenuItem.Visible = False
            Case "Customer Parts Received"
...
WorkOrderPartsTable.WorkOrderRequestedPartsHeaderID_LongInteger, 
        ...
        For i = 0 To WorkOrderPartsPerJobDataGridView.Columns.GetColumnCount(0) - 1
            ...
            Select Case WorkOrderPartsPerJobDataGridView.Columns.Item(i).Name
                ...
                Case "Quantity_Integer"
                    ...
                    WorkOrderPartsPerJobDataGridView.Columns.Item(i).Visible = True
                Case "OrderedProductsPartsTable.ManufacturerPartNo_ShortText30Fld"
                    If Not PartsFromCustomer Then
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).HeaderText = "Part No."
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).Width = 170
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).Visible = True
                    End If
                Case "OrderedProductsPartsTable.ManufacturerDescription_ShortText250"
                    If Not PartsFromCustomer Then
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).HeaderText = "Product Ordered"
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).Width = 400
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).Visible = True
                    End If
                Case "ProductsPartsTable.ManufacturerPartNo_ShortText30Fld"
                    If PartsFromCustomer Then
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).HeaderText = "Part No."
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).Width = 170
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).Visible = True
                    End If
                Case "ProductsPartsTable.ManufacturerDescription_ShortText250"
                    If PartsFromCustomer Then
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).HeaderText = "Received from Customer"
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).Width = 400
                        WorkOrderPartsPerJobDataGridView.Columns.Item(i).Visible = True
                    End If
                Case "StatusText_ShortText25"
...
Str(CurrentConcernJobStatusID)
    ...
    Private Sub ShowRequestPartsForm()
        ...
        RequestPartsForm.VehicleNameButton.Text = WorkOrdersDataGridView.Item("VehicleDescription", CurrentWorkOrdersRow).Value
    ...
    Private Sub EnableConcernMenus()
        ...
        RemoveConcernToolStripMenuItem.Visible = True
    End Sub
    Private Sub DisableConcernMenus()
    ...
    Private Sub EnableJobMenus()
        AddJobToolStripMenuItem.Visible = True
        EditJobToolStripMenuItem.Visible = True
        If CurrentUserGroup = "Lead Service Specialist" Then AssignJobToolStripMenuItem.Visible = True
    End Sub
    Private Sub DisableJobMenus()
        AddJobToolStripMenuItem.Visible = False
        EditJobToolStripMenuItem.Visible = False
    ...
    Private Sub AssignWorkOrderToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AssignWorkOrderToolStripMenuItem.Click
        If WorkOrderConcernJobsRecordCount < 1 Then
            MsgBox("Attach Job(s) before you can assign Work Order")
        End If
    ...
    Private Sub AssignConcernToolStripMenuItem2_Click(sender As Object, e As EventArgs) Handles AssignConcernToolStripMenuItem.Click
        If WorkOrderConcernJobsRecordCount < 1 Then
            MsgBox("Attach JOB(S) first to this CONCERN before doing this menu")
            Exit Sub
        End If
    ...
    Private Sub GetStandardJobsToolStripMenuItem_Click_1(sender As Object, e As EventArgs) Handles GetStandardJobsToolStripMenuItem.Click
