...
" & Chr(34) & "CONCERNS" & Chr(34) &
    ...
    Private Sub InsertIntoConcernsTable()
        ...
        End If
        If CurrentMasterCodeBookID > -1 Then
            Dim FieldsToUpdate = " InformationsHeadersTypeID_LongInteger, " &
                                    " Source_Byte, " &
                                    " MasterCodeBookID_LongInteger "
            Dim FieldsData = Str(CurrentConcernTypeID) & ", " &
                                    Str(2) & ", " &
                                    Str(CurrentMasterCodeBookID)
            CurrentConcernID = InsertNewRecord("ConcernsTable", FieldsToUpdate, FieldsData)
        Else
            Dim FieldsToUpdate = " LongTextConcern_LongText "
            Dim FieldsData = InQuotes(ConcernTextBox.Text)
            CurrentConcernID = InsertNewRecord("ConcernAsIsByClientTable", FieldsToUpdate, FieldsData)
        End If
        FillConcernsDataGridView()
    End Sub
