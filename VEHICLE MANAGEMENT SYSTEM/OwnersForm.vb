Public Class OwnersForm
    ...
    Private Sub LoadOwnerDetails()
        LastNameTextBox.Text = NotNull(OwnerDataGridView.Item("LastName_ShortText15", CurrentOwnerDataGridViewRow).Value)
        FirstNameTextBox.Text = NotNull(OwnerDataGridView.Item("FirstName_ShortText15", CurrentOwnerDataGridViewRow).Value)
...
"
    ...
    Private Function AChangeOccuredInOwnerDetails()
        If TheseAreNotEqual(LastNameTextBox.Text, OwnerDataGridView.Item("LastName_ShortText15", CurrentOwnerDataGridViewRow).Value, PurposeOfEntry) Then Return True
        If TheseAreNotEqual(FirstNameTextBox.Text, OwnerDataGridView.Item("FirstName_ShortText15", CurrentOwnerDataGridViewRow).Value, PurposeOfEntry) Then Return True
    ...
    Private Sub InsertNewOwnerRecord()
        ...
        MySelection = "Select TOP 1 * FROM OwnersTable WHERE LastName_ShortText15  = " & Chr(34) & LastNameTextBox.Text & Chr(34) &
                        " AND FirstName_ShortText15 = " & Chr(34) & FirstNameTextBox.Text & Chr(34)
        ...
        If RecordIsFound() Then
            ...
            If MsgBox(xxMessage & "is existing " & vbCrLf & " is the customer's phone number ?", vbYesNo) = vbYes Then
                OwnerDetailsGroupBox.Visible = False
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles ExitSaveVehicleChangesButton.Click
        If Not AChangeOccuredInVehicleDetails() Then
            If MsgBox("You have no new entries to save, CANCEL Editing ?", MsgBoxStyle.YesNo) = vbNo Then
                OwnerDetailsGroupBox.Visible = False
                ServicedVehicleDetailsGroupBox.Visible = True
                Exit Sub
            End If
