Public Class WorkOrderFormCRS
    ...
    Private Sub FillWorkOrdersDataGridView()
        ...
        Dim OwnerName = " (trim(OwnersTable.FirstName_ShortText15) & space(1) & Trim(OwnersTable.LastName_ShortText15) & space(1) & trim(OwnersTable.NamePrefix_ShortText3)),"
        ...
        Dim VehicleReleaseDate = " WorkOrdersTable.ReleaseDate_DateTime, "
        Dim EmailAddress = " OwnersTable.EmailAddress_ShortText20, "
        Dim Address = "(trim(OwnersTable.Street_ShortText25) & 
                        trim(OwnersTable.BldgAptRmNo_ShortText25) &  "", "" & 
                        Trim(CityTable.CityName_ShortText25) & "", "" & 
                        Trim(CityTable.ZipCode_ShortText9))"
        Dim xxPending = "IIf(WorkOrdersTable.WorkOrderStatus_Byte = 0, " & Chr(34) & " pending " & Chr(34) & ","
        Dim xxForAnalysis = "IIf(WorkOrdersTable.WorkOrderStatus_Byte = 1, " & Chr(34) & " for analysis " & Chr(34) & ","
        Dim xxForAssignment = "IIf(WorkOrdersTable.WorkOrderStatus_Byte = 2, " & Chr(34) & " for assignment " & Chr(34) & ","
        Dim xxForBilling = "IIf(WorkOrdersTable.WorkOrderStatus_Byte = 3, " & Chr(34) & " for billing " & Chr(34) & ","
        Dim WorkStatus = xxPending & xxForAnalysis & xxForAssignment & xxForBilling & Chr(34) & Chr(34) & " )))), "
        ...
        WorkOrdersTablesLinks = " FROM (((((((((WorkOrdersTable LEFT JOIN ServicedVehiclesTable ON WorkOrdersTable.[ServicedVehicleID_LongInteger] = ServicedVehiclesTable.ServicedVehicleID_AutoNumber) " &
                                                            ...
                                                            " LEFT JOIN (VehiclesTable LEFT JOIN VehicleModelsRelationsTable On VehiclesTable.[VehicleModelsRelationID_LongInteger] = VehicleModelsRelationsTable.[VehicleModelsRelationID_Autonumber]) On ServicedVehiclesTable.VehicleID_LongInteger = VehiclesTable.VehicleID_AutoNumber) " &
                                                            ...
                                                            " LEFT JOIN PersonnelTable On WorkOrdersTable.AssignedLeadMechanic_longInteger = PersonnelTable.PersonnelID_AutoNumber) " &
                                                            " LEFT JOIN PersonnelTable As PersonnelTable_1 On WorkOrdersTable.ServiceManagerAssigning_LongInteger = PersonnelTable_1.PersonnelID_AutoNumber) " &
                                                            " LEFT JOIN PersonnelTable As PersonnelTable_2 On WorkOrdersTable.CustomerServiceInCharge_LongInteger = PersonnelTable_2.PersonnelID_AutoNumber "

        MySelection = WorkOrderFieldsToSelect & WorkOrdersTablesLinks & WorkOrderSelectionFilter & WorkOrderSelectionOrder

        WorkOrdersTable.MyDbCommand(MySelection)
        If NotEmpty(WorkOrdersTable.Exception) Then MsgBox(WorkOrdersTable.Exception) : Exit Sub
        ...
        End If
        WorkOrdersDataGridView.DataSource = WorkOrdersTable.MyAccessDbDataTable
    ...
    Private Sub FormatWorkOrdersDataGridView()
        ...
        Dim cccdf = WorkOrdersDataGridView.Columns.GetColumnCount(0)
        For i = 0 To WorkOrdersDataGridView.Columns.GetColumnCount(0) - 1

            Dim DoNotDisplayFields = "OwnerID_AutoNumberPlateNumber_ShortText20VinNo_ShortText20WorkOrderID_AutoNumberWorkOrderStatus_ByteCustomerServiceInCharge_LongIntegerServiceManagerAssigning_LongIntegerAssignedLeadMechanic_longInteger"
            ...
            Select Case WorkOrdersDataGridView.Columns.Item(i).HeaderText
                ...
                Case "EmailAddress_ShortText20"
                    ...
                    WorkOrdersDataGridView.Columns.Item(i).Width = 100
                Case "Expr1004"
                    WorkOrdersDataGridView.Columns.Item(i).Name = "VehicleName"
                    WorkOrdersDataGridView.Columns.Item("VehicleName").HeaderText = "Vehicle Description"
                    WorkOrdersDataGridView.Columns.Item("VehicleName").Width = 180
                Case "Expr1005"
                    WorkOrdersDataGridView.Columns.Item(i).Name = "Customer"
                    WorkOrdersDataGridView.Columns.Item("Customer").HeaderText = "Customer"
                    WorkOrdersDataGridView.Columns.Item("Customer").Width = 160
                Case "Expr1009"
                ...
                Case "Expr1011"
                    ...
                    WorkOrdersDataGridView.Columns.Item(i).Width = 160
                Case "Expr1012"
                    WorkOrdersDataGridView.Columns.Item(i).Name = "WorkOrderStatus"
                    WorkOrdersDataGridView.Columns.Item("WorkOrderStatus").Width = 117
                    WorkOrdersDataGridView.Columns.Item("WorkOrderStatus").HeaderText = "Status"
                Case "Expr1014"
                    WorkOrdersDataGridView.Columns.Item("WorkOrderStatus").HeaderText = "Status"
...
"
    ...
    Private Sub FillWorkOrderConcernsDataGridViewOLD()
        'NOTE IF MasterCodeBookTable.SystemDesc_ShortText100Fld IS EMPTY IT MEANS 
        ' ConcernAsIsByClientTable.LongTextConcern_LongText

        Dim ActionToTake = " IIf( Not MasterCodeBookTable.SystemDesc_ShortText100Fld = " & Chr(34) & Chr(34) & ",  " &
                                 " InformationsHeadersTypeTable.InformationsHeadersTypePrefix_ShortText50, " &
                                 Chr(34) & " Diagnose " & Chr(34) & ") "

        Dim ConcernDescription = " IIf( Not MasterCodeBookTable.SystemDesc_ShortText100Fld = " & Chr(34) & Chr(34) & ",  " &
                                 " MasterCodeBookTable.SystemDesc_ShortText100Fld, " &
                                 " ConcernAsIsByClientTable.LongTextConcern_LongText), "
        ...
        WorkOrderConcernsFieldsToSelect = " Select " &
                                          ...
                                          ConcernDescription &
                                          " ConcernsTable.Concern_ShortText255, " &
                                          " ConcernsTable.Source_Byte "

        WorkOrderConcernsTablesLinks = " FROM ((WorkOrderConcernsTable LEFT JOIN (ConcernsTable LEFT JOIN MasterCodeBookTable On ConcernsTable.MasterCodeBookId_LongInteger = MasterCodeBookTable.MasterCodeBookID_Autonumber) On WorkOrderConcernsTable.ConcernID_LongInteger = ConcernsTable.ConcernID_AutoNumber) LEFT JOIN ConcernAsIsByClientTable On WorkOrderConcernsTable.ConcernLongTextCodeID_LongInteger = ConcernAsIsByClientTable.LongTextConcernID_Autonumber) LEFT JOIN InformationsHeadersTypeTable On ConcernsTable.InformationsHeadersTypeID_LongInteger = InformationsHeadersTypeTable.InformationsHeadersTypeID_AutoNumber  "
        WorkOrderConcernsSelectionFilter = " WHERE WorkOrderID_LongInteger = " & Str(CurrentWorkOrderID)
        WorkOrderConcernsSelectionOrder = " "
    ...
    Private Sub FormatWorkOrderConcernsDataGridViewOLD()

        Dim HeightToAdd = IIf(WorkOrderConcernsRecordCount = 0, 0, ((WorkOrderConcernsRecordCount) * DataGridViewRowHeight))
        WorkOrderConcernsDataGridView.Height = WorkOrderConcernsDataGridView.ColumnHeadersHeight + HeightToAdd + 100

        Dim DoNotDisplayFields = " WorkOrderConcernID_AutoNumber, " &
        ...
        For i = 0 To WorkOrderConcernsDataGridView.Columns.GetColumnCount(0) - 1
            ...
            If InStr(DoNotDisplayFields, WorkOrderConcernsDataGridView.Columns.Item(i).HeaderText) > 0 Then
                WorkOrderConcernsDataGridView.Columns.Item(i).Visible = False
            End If
        ...
        If WorkOrderConcernsDataGridView.Width > Me.Width Then
            WorkOrderConcernsDataGridView.Width = Me.Width - 100
        Else
        ...
        WorkOrderConcernsDataGridView.Top = 30
        Me.Left = (VehicleManagementSystemForm.Width - Me.Width) / 2
        Me.Location = New Point(Me.Location.X, 55)
        WorkOrdersDataGridView.Left = (Me.Width - WorkOrdersDataGridView.Width) / 2
        '===========================
...
"
        ...
        If RecordCount > 0 Then AllWorkOrderPartsDataGridView.Visible = True
    ...
    Private Sub WorkOrderConcernsDataGridView_RowEnter(sender As Object, e As DataGridViewCellEventArgs) Handles WorkOrderConcernsDataGridView.RowEnter
        ...
        If WorkOrdersDataGridView.Item("WorkOrderStatus_Byte", CurrentWorkOrderRow).Value > 0 Then
    ...
    Public Sub SetupWorkOrderSelection(WorkOrderStatusSelection)
        ...
        Select Case WorkOrderStatusSelection
            Case 0
                WorkOrderSelectionFilter = " WHERE (WorkOrderStatus_Byte  = 0 OR WorkOrderStatus_Byte = 3)"
                Me.Text = "OUTSTANDING WORK ORDERS"
            Case 1
                WorkOrderSelectionFilter = " WHERE WorkOrderStatus_Byte = 1 "
            Case 2
                WorkOrderSelectionFilter = " WHERE WorkOrderStatus_Byte = 2 "
            Case 3
                WorkOrderSelectionFilter = " WHERE WorkOrderStatus_Byte = 3 "
    ...
    Private Sub WorkOrdersDataGridView_RowEnter(sender As Object, e As DataGridViewCellEventArgs) Handles WorkOrdersDataGridView.RowEnter
        ...
        If CurrentWorkOrderRow < 12 Then
            WorkOrderConcernsDataGridView.Top = WorkOrdersDataGridView.Top + (WorkOrdersDataGridView.Rows(0).Height * 12)
        Else
            WorkOrderConcernsDataGridView.Top = WorkOrdersDataGridView.Top
        End If
        ...
        Select Case CurrentUserGroup
            ...
            Case "Customer Relations Specialist"
                ...
                Select Case CurrentWorkOrderStatus
                    Case 0 ' Outstanding / Temporary entry status 
                        SubmitForServiceToolStripMenuItem.Visible = False
                    Case 3
                        DisableEdit()
                        FinalizeToolStripMenu.Visible = True
                        PrintWorkOrderToolStripMenuItem.Visible = True  'Set To visible When record status = 3 For billing
                End Select
            ...
            Case "Assistant Service Manager"
                ...
                For i = 0 To WorkOrderConcernsRecordCount - 1
                    If Not WorkOrderConcernsDataGridView.Item("WorkOrderConcernStatus_Byte", i).Value = 2 Then
    ...
    Private Sub LoadWorkOrderDetails()
        ...
        CustomerTextBox.Text = WorkOrdersDataGridView.Item("Customer", CurrentWorkOrderRow).Value
        ...
        End If
        VehicleDetailsTextBox.Text = WorkOrdersDataGridView.Item("VehicleName", CurrentWorkOrderRow).Value
    ...
    Private Sub WorkOrderForm_EnabledChanged(sender As Object, e As EventArgs) Handles Me.EnabledChanged
        ...
        Select Case Tunnel1
            ...
            Case "Tunnel2IsCustomerID"
                CurrentCustomerID = Tunnel2 ' i think this is not needed. customer info can be obtained through the usedVehicleId
                CurrentServicedVehicleID = Tunnel4
            Case "Tunnel2IsCurrentPersonnelID"
    ...
    Private Sub AssignThisWorkOrder()
        ...
        Select Case CurrentUserGroup
            Case "3B"
                WorkOrderFieldsToSelect = " UPDATE WorkOrdersTable  SET " &
    ...
    Private Sub WorkOrderDetailsGroup_VisibleChanged(sender As Object, e As EventArgs) Handles WorkOrderDetailsGroup.VisibleChanged
        If Not CurrentUserGroup = "Customer Relations Specialist" Then
            Exit Sub
        End If
